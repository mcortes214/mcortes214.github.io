//https://cdn.rawgit.com/mrvautin/typewrite/master/dist/typewrite.min.js
...
(function($) {
    $.fn.typewrite = function(options) {
        ...
        function processActions() {
            actions.forEach(function(element, index) {
                ...
                if (Object.keys(element).includes("type")) {
                    ...
                    //ESTO ESTÁ MAL PERO ES UN HACK QUE NECESITO
                    // else if(element.type === "[abrir linea final]"){
                    //   abrirLineaFinal()
                    // }
                    // else if(element.type === "[cerrar linea final]"){
                    //   cerrarLineaFinal()
                    // }
        ...
        //HACK
        function abrirLineaFinal() {
            $(settings.el).delay(settings.speed).queue(function(next) {
                var currTextNoCurr = $(this).html().substring(0, $(this).html().length);
                $(this).html(currTextNoCurr + '<span class="typewrite-large">');
                next();
                settings.queue = settings.queue - 1;
                $(settings.mainEl).trigger("typewriteNewLine")
            })
        ...
        //HACK
        function cerrarLineaFinal() {
            $(settings.el).delay(settings.speed).queue(function(next) {
                var currTextNoCurr = $(this).html().substring(0, $(this).html().length);
                $(this).html(currTextNoCurr + '</span>');
                next();
                settings.queue = settings.queue - 1;
                $(settings.mainEl).trigger("typewriteNewLine")
            })
        ...
        //HACK
        function abrirLinkFinal() {
            $(settings.el).delay(settings.speed).queue(function(next) {
                var currTextNoCurr = $(this).html().substring(0, $(this).html().length);
                $(this).html(currTextNoCurr + '<a href="ficciones-y-obras.html">');
                next();
                settings.queue = settings.queue - 1;
                $(settings.mainEl).trigger("typewriteNewLine")
            })
        ...
        //HACK
        function cerrarLinkFinal() {
            $(settings.el).delay(settings.speed).queue(function(next) {
                var currTextNoCurr = $(this).html().substring(0, $(this).html().length);
                $(this).html(currTextNoCurr + '</a>');
                next();
                settings.queue = settings.queue - 1;
                $(settings.mainEl).trigger("typewriteNewLine")
            })
